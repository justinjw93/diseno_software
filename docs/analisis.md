...
El sistema evolucionó desde el análisis narrativo del caso de tutorías hacia un modelo orientado a objetos donde cada clase resuelve una única responsabilidad (`Usuario`/`Estudiante`/`Docente`, `HorarioTutoria`, `Reserva`) y donde las dependencias hacia infraestructura (persistencia y notificaciones) quedaron detrás de interfaces (`RepositorioReservas`, `RepositorioHorarios`, `Notificador`), siguiendo Dependency Inversion. La cohesión alta y el acoplamiento bajo no son una declaración teórica: se verifican en que `ServicioReservas` puede probarse por completo con implementaciones en memoria y un notificador de prueba, y en que agregar `NotificadorLog` no requirió tocar ninguna clase existente. El historial de Git documenta esa evolución de forma incremental (dominio → reservas → persistencia → servicio y notificaciones → pruebas → una segunda implementación de notificación → documentación), en lugar de haberse escrito todo en un único commit final.
